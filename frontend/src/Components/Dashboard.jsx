...
const Dashboard = () => {
    ...
    const onSaveEdit = async () => {
        ...
        await dispatch(updateSubs({ id: selectedSub.id, subData: editForm }))
        closeEdit()
    }
    ...
    const confirmDelete = async () => {
        if (!toDelete) return
        await dispatch(deleteSubs(toDelete.id))
        closeDelete()
    }
            ...
            <section className="min-h-screen bg-gray-50 dark:bg-gray-900">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="relative overflow-x-auto rounded-lg shadow-md">
                            ...
                            <tbody>
                                {
                                    subs.length === 0 ? (
                                        <tr>
                                            <td colSpan="7" className="px-6 py-4 text-center text-gray-900 dark:text-white">
                                                No subscriptions found.
                                            </td>
                                        </tr>
                                    ) : (
                                        subs.filter(sub => sub.name.toLowerCase().includes(search.toLowerCase())).map((sub) => (
                                            <tr key={sub.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <div id="editUserModal" tabIndex="-1" aria-hidden={!isEditOpen} className={`fixed inset-0 z-50 ${isEditOpen ? 'flex' : 'hidden'} items-center justify-center w-full p-4 overflow-x-hidden overflow-y-auto`}>
