...
exports.register = async (req, res) => {
  try {
    ...
    // Set HTTP-only cookie and send token in response
    res.cookie('auth_token', token, getCookieOptions());
    
    return res.status(201).json({ 
      user: { id: user._id, email: user.email, role: user.role, name: user.name },
      token // Send token in response body for cross-domain use
    });
  } catch (e) {
...
exports.login = async (req, res) => {
  try {
    ...
    // Set HTTP-only cookie (for same-domain) and send token in response (for cross-domain)
    res.cookie('auth_token', token, getCookieOptions());
